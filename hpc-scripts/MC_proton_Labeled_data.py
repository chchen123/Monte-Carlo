...
output_path = '/home/chen/ar46/MonteCarlo/run_'+run+'.h5'
...
with h5py.File(output_path, 'w') as outFile:
    ...
    gp4 = outFile.require_group('proton')
    for event_index in range(len(evt_inFile)):
    #for event_index in range(1002):
        try:
        ...
        try:
            ...
            if np.isnan(mcres['enChi2']) == True: # disregard the NaN results
                ...
                if event_index in p_indices:
                    chi_proton = np.append(chi_proton,[mcres['posChi2']+mcres['enChi2']+mcres['vertChi2']])
        except Exception:
            continue        
        ...
        try:
            ...
            dset4 = gp4.create_dataset('{:d}'.format(event_index), data=chi_proton, compression='gzip')
        except Exception:
            continue
