        ...
        for evt_id in range(n_evts):
            try:
                ...
                try:
                    ...
                    print('Event deleted: non-physical evet', str(evt_id))
                    continue
                            
                del_list = []
                for i in range(len(curxyz)):
                    #disregard the points that have time bucket index>500
                    if (curxyz[i][2])*CLOCK/DRIFT_VEL > 500.0:
                        del_list.append(i)
                    ...
                    elif curxyz[i][6] > 40.0:
                        del_list.append(i)
                curxyz = np.delete(curxyz, del_list, axis=0)
